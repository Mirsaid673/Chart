...
void Chart::start() {
    ...
    auto algos_status = m_stub->GetAlgos(&algo_context, algo_request, &algo_response);
    ...
    for (auto& alg : algo_response.algos())
    ...
    for (const auto& algo : m_algos) {
        ...
        auto streams_status = m_stub->GetStreamsByAlgo(&streams_context, streams_request, &streams_response);
    ...
    // for (int i = 0; i < 100;i++) {
    //     chart_api::DataPoint candle;
    //     m_current_stream_data.emplace_back();
    // }
...
std::vector<chart_api::DataPoint> Chart::getStreamData(int32_t stream_id) {
    ...
    const chart_api::GetDataRequest request;
    chart_api::GetDataResponse response;
    auto status = m_stub->GetData(&context, request, &response);
...
void Chart::draw() {
    ...
    if (first_time)
    {
        ...
        ImGui::DockBuilderDockWindow(m_algos_title.c_str(), right);
        ImGui::DockBuilderDockWindow(m_streams_title.c_str(), right);
...
void Chart::contentWindow() {
    ...
    ImGui::Begin(m_content_title.c_str());
    ...
    if (ImPlot::BeginPlot("Candlestick Chart", {-1,-1}, ImPlotFlags_CanvasOnly)) {
    ...
    plotCandles("candles", m_current_stream_data);
        ImPlot::EndPlot();
    }
    ImGui::End();
}

void Chart::algosWindow() {
    ImGuiWindowClass window_class;
    window_class.DockNodeFlagsOverrideSet = ImGuiDockNodeFlags_NoWindowMenuButton | ImGuiDockNodeFlags_NoUndocking;
    ImGui::SetNextWindowClass(&window_class);
    ImGui::Begin(m_algos_title.c_str(), nullptr, ImGuiWindowFlags_NoFocusOnAppearing);
...
void Chart::streamsWindow() {
    ImGuiWindowClass window_class;
    window_class.DockNodeFlagsOverrideSet = ImGuiDockNodeFlags_NoWindowMenuButton | ImGuiDockNodeFlags_NoUndocking  ;
    ImGui::SetNextWindowClass(&window_class);
    ImGui::Begin(m_streams_title.c_str(), nullptr, ImGuiWindowFlags_NoFocusOnAppearing);

    for (const auto&[id, stream] : m_streams) {
        if (stream.algo() != m_algos[m_current_algo])
            continue;
        std::string label = stream.coin() + '/' + stream.coin_second();
...
size_t to_time(const std::string& str) {
    return std::stoi(str);
}

void Chart::plotCandles(const std::string &label, const std::vector<chart_api::DataPoint> &candles) { ImDrawList* draw_list = ImPlot::GetPlotDrawList();
    ...
    auto count = candles.size();
    double half_width = count > 1 ? (to_time(candles[1].time()) - to_time(candles[1].time())) * candle_width : candle_width;
    ...
    if (ImPlot::BeginItem(label.c_str())) {
        ...
        for (const auto& candle : candles) {
            for (int i = 0; i < count; ++i) {
                ImPlot::FitPoint(ImPlotPoint(to_time(candle.time()), candle.low()));
                ImPlot::FitPoint(ImPlotPoint(to_time(candle.time()), candle.high()));
            }
        }
        for (const auto& candle : candles) {
            ImVec2 open_pos  = ImPlot::PlotToPixels(to_time(candle.time()) - half_width, candle.open());
            ImVec2 close_pos = ImPlot::PlotToPixels(to_time(candle.time()) + half_width, candle.close());
            ImVec2 low_pos   = ImPlot::PlotToPixels(to_time(candle.time()), candle.low());
            ImVec2 high_pos  = ImPlot::PlotToPixels(to_time(candle.time()), candle.high());
